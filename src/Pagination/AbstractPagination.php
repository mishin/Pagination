 ...
 * @see (-) createPage($pageNumber, $isCurrentPage = false);
 * @see (+) setCurrentPageNumber($currentPageNumber = null);
 * @see (+) setLimitPerPageOffset(\Closure $limitPerPageOffset);
 * @see (+) recalculate(array $settings);
 * @see (+) getLimitPerPageOffset(\Closure $overridePerPageOffset = null, $newPage = null);
 *
...
abstract class AbstractPagination implements PaginationInterface, ServiceFunctionsInterface
{
    ...
    public function __construct(array $settings)
    {
        ...
        if ($this->currentPageNumber < static::BASE_PAGE) {
    ...
    public function recalculate(array $settings)
    {
        ...
        if ($this->maxPagesToShow <= 3) {
            throw new \Exception('Number of neighboring pages must be at least 3');
        }

        if ($this->itemsPerPage <= 0) {
            throw new \Exception('Items per page must be at least 1');
        }
    ...
    {
        return $this->currentPageNumber > $this->pageCount ? static::BASE_PAGE : (int) $this->getProperty('currentPageNumber');
    }
    ...
    public function getPageUrl($pageNumber)
    {
        if (!is_int($pageNumber)) {
            throw new \InvalidArgumentException('$pageNumber must be passed as a integer');
        }
    ...
    public function renderAsArray()
    {
        ...
        if ((int) $this->pageCount <= (int) $this->maxPagesToShow) {
            ...
            $numberAdjacents = (int) floor(((int) $this->maxPagesToShow - 3) / 2);

            if ((int) $this->currentPageNumber + $numberAdjacents > (int) $this->pageCount) {
     ...
     *
     * @throws \InvalidArgumentException on non integer {$pageNumber}
     * @throws \InvalidArgumentException on non boolean {$isCurrentPage}
     *
     * @return array
     */
    protected function createPage($pageNumber, $isCurrentPage = false)
    {
        if (!is_bool($isCurrentPage)) {
            throw new \InvalidArgumentException('$isCurrentPage must be passed as a boolean');
        }
    ...
    {
        return [
            'pageNumber'    => static::NAVIGATION_ELLIPSES,
            'pageUrl'       => null,
            'isCurrentPage' => false,
        ];
    }
    ...
    public function renderLargePaging()
    {
        ...
        $html = sprintf('<ul class="pagination">%s', "\n");
